...
// 3. Update stopCampaign and others to fetch history
...
// Add fetchHistory to startCampaign
c = c.replace(
  "      await fetchStatus();\n    } catch (error) {",
  "      await fetchStatus();\n      await fetchHistory();\n    } catch (error) {"
);

// Add fetchHistory to resumeCampaign
c = c.replace(
  "      await fetchStatus();\n    } catch (error) {",
  "      await fetchStatus();\n      await fetchHistory();\n    } catch (error) {"
);

// 4. Update the "Campaign Status" UI box to support multiple
...
const oldCampaignBoxStr = c.substring(c.indexOf(oldCampaignBoxStart), c.indexOf(oldCampaignBoxEnd) + oldCampaignBoxEnd.length);
